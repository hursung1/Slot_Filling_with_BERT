...
from src.datareader import datareader, read_file, data_binarize
...
def test_on_seen_and_unseen(params, model, BIOTagger, tokenizer, cuda_available):
    ...
    if unseen_data_bin:
        ...
        _, _, test_f1 = eval(model, BIOTagger, tokenizer, unseen_dataloader, cuda_available, True)
    ...
    if seen_data_bin:
        ...
        _, _, test_f1 = eval(model, BIOTagger, tokenizer, seen_dataloader, cuda_available, True)
...
if __name__ == "__main__":
    ...
    if params.use_plain:
        bert_model_path = f'./experiments/Bert/Plain/{params.tgt_dm}/Sample{params.n_samples}/Epoch{params.epoch}/'
        tagger_model_path = f'./experiments/Tagger/Plain/{params.tgt_dm}/Sample{params.n_samples}/Epoch{params.epoch}/state_dict_model.pt'
        tokenizer = BertTokenizer.from_pretrained(bert_model_path)
        model = BertModel.from_pretrained(bert_model_path)

    else:
        bert_model_path = f'./experiments/Bert/NER_Pretrained/{params.tgt_dm}/Sample{params.n_samples}/Epoch{params.epoch}/'
        tagger_model_path = f'./experiments/Tagger/NER_Pretrained/{params.tgt_dm}/Sample{params.n_samples}/Epoch{params.epoch}/state_dict_model.pt'
