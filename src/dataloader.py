...
class Dataset(data.Dataset):
    def __init__(self, X, y1, y2, domains, tokenizer):
        self.X = X
        self.y1 = y1 
        self.y2 = y2
...
def get_dataloader(tgt_domain, batch_size, n_samples, tokenizer):
    ...
    dataset_tr = Dataset(train_data["utter"], train_data["y1"], train_data["y2"], train_data["domains"], tokenizer)
    dataset_val = Dataset(val_data["utter"], val_data["y1"], val_data["y2"], val_data["domains"], tokenizer)
    dataset_test = Dataset(test_data["utter"], test_data["y1"], test_data["y2"], test_data["domains"], tokenizer)
